...
# End-to-end: STEP import -> mesh -> BCs -> modal job -> frequency results
...
# ============================================================
STEP_FILE   = 'C:/Users/admin/Downloads/wr3400.step'  # <-- change path
...
# -------------------------------------------------------
# STEP 1: Import STEP file into new model
# -------------------------------------------------------
print("--- Step 1: Importing STEP file ---")

if not os.path.exists(STEP_FILE):
    print("ERROR: STEP file not found at:", STEP_FILE)
    print("Please update the STEP_FILE path at the top of the script.")
    raise SystemExit

# Clear any existing models
for mname in list(mdb.models.keys()):
    if mname != MODEL_NAME:
        try:
...
# Import STEP
mdb.openStep(STEP_FILE, scaleFromFile=OFF)

# Abaqus imports STEP into a new model — find it
imported_model_name = None
for mname in mdb.models.keys():
    if mname != MODEL_NAME:
        imported_model_name = mname
        break

if imported_model_name:
    # Rename imported model to Model-1
    mdb.models.changeKey(
        fromName=imported_model_name,
        toName=MODEL_NAME
    )
    model = mdb.models[MODEL_NAME]
    print("  STEP imported into:", MODEL_NAME)
else:
    model = mdb.models[MODEL_NAME]
